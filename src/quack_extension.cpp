            ...
            try {
                ...
                if (dataset_j.contains("measures")) {
                    for (const auto& measure_json : dataset_j["measures"]) {
                        ...
                        measure.aggregation_type = measure_json.contains("type") ? 
                            measure_json["type"].get<string>() : "sum";
                ...
                return StringVector::AddString(result, "Dataset '" + dataset_name.GetString() + "' registered successfully");
...
void RegisterSemanticQueryFunctions(DatabaseInstance &instance) {
    ...
    TableFunction semantic_query_func("SEMANTIC_QUERY", {LogicalType::VARCHAR}, SemanticQueryFunction, SemanticQueryBind);
    ...
    auto register_dataset_function = ScalarFunction("REGISTER_DATASET", 
